	...
	if not is_on_floor() and gravity > 0:
		...
		if splatted:
			...
			if squashTween == null or not squashTween.is_running():
				...
				squashTween.tween_property(self,"scale", Vector3(0.8,2.2,0.8), 1)
		...
		squashTween.tween_property(self,"scale", Vector3(1.3,1.5,1.3), 0.1)
		squashTween.tween_property(self,"scale", Vector3(1,2,1), 0.2)
